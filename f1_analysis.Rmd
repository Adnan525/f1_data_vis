...
pit_with_year <- inner_join(races, pit_stops, by = "raceId") %>% 
  select(raceId, year, round, circuitId, driverId, stop, lap, milliseconds)
```

```{r}
target <- pit_with_year %>%
  group_by(year) %>%
  summarize(fastest_lap = min(milliseconds))
...
target <- bind_rows(
  target %>% filter(year > 2021), # data after 2021
  temp,
  temp2
)
...
```{r}
plot(temp$year, temp$fastest_lap, type = "l", 
     xlab = "Race ID", ylab = "Max Duration", 
     main = "Maximum Pit Stop Duration by Race ID")
